...
def check_latitude_rad(latitude):
     if not MINLAT_RADIANS <= latitude <= MAXLAT_RADIANS:
        raise ValueError(
            'latitude outside valid range {0!r} to {1!r} rad: {2!r}'
            .format(MINLAT_RADIANS, MAXLAT_RADIANS, latitude))
...
def check_sol_dec_rad(sd):
    ...
    """
    if not MINSOLDEC_RADIANS <= sd <= MAXSOLDEC_RADIANS:
        raise ValueError(
            'solar declination outside valid range {0!r} to {1!r} rad: {2!r}'
            .format(MINSOLDEC_RADIANS, MAXSOLDEC_RADIANS, sd))
...
def check_sunset_hour_angle_rad(sha):
    ...
    See http://mypages.iit.edu/~maslanka/SolarGeo.pdf
    """
    if not MINSHA_RADIANS <= sha <= MAXSHA_RADIANS:
        raise ValueError(
            'sunset hour angle outside valid range {0!r} to {1!r} rad: {2!r}'
            .format(MINSHA_RADIANS, MAXSHA_RADIANS, sha))
#########################################################################################
"""
...
# Internal constants
# Latitude
MINLAT_RADIANS = deg2rad(-90.0)
MAXLAT_RADIANS = deg2rad(90.0)

# Solar declination
MINSOLDEC_RADIANS = deg2rad(-23.5)
MAXSOLDEC_RADIANS = deg2rad(23.5)

# Sunset hour angle
MINSHA_RADIANS = 0.0
MAXSHA_RADIANS = deg2rad(180)

############################################################################

#service: python_script.evapotranspiration
#data_template: 
#  latitude: 49.38174766529562
#  elevation: 100
#  doy: '{{ now().timetuple().tm_yday }}'
#  wind: 'sensor.wind_mean'
#  temp: 'sensor.temp_draussen_mean'
#  temp_min: 'sensor.temp_draussen_min'
#  temp_max: 'sensor.temp_draussen_max'
#  dew: 'sensor.taupunkt_draussen'
#  pressure: 'sensor.lumi_lumi_weather_034d16f5_1_1027'
#  solar: 'sensor.solar_radiation_24h'

#day_of_year = datetime.datetime.now().timetuple().tm_yday
#day_of_year=186
#day_of_year=hass.states.get("doy")
day_of_year=data.get("doy",1)
#logger.error(day_of_year)
#logger.error(data.get("wind"))
windsensor=data.get("wind")
#logger.error(hass.states.get(windsensor).state)
if hass.states.get(windsensor).state:
    ws=float(hass.states.get(windsensor).state) / 3.6 
else:
    ws=0
    logger.error("evapotranspiration line 890: no wind")

#logger.error(ws)
#logger.error(hass.states.get("zone.home"))

latitude = data.get("latitude")
elevation = data.get("elevation")
#latitude = 49.381932
#elevation=100 #100m hoch

temp_c_min=float(hass.states.get(data.get("temp_min")).state)
temp_c_max=float(hass.states.get(data.get("temp_max")).state)
#temp_c = (temp_c_min + temp_c_max) / 2.0
temp_c=float(hass.states.get(data.get("temp")).state)
#ws=7/3.6  # windspeed
xdew=hass.states.get(data.get("dew")).state
tdew=float(xdew) if xdew.replace('.','',1).isdigit() else 9999
atmos_pres=float(hass.states.get(data.get("pressure")).state)
sol_rad=float(hass.states.get(data.get("solar")).state)
#sol_rad=26.7
#logger.error("xdew: %s",xdew)
#logger.error("tdew: %f",tdew)

sha = sunset_hour_angle(deg2rad(latitude), sol_dec(day_of_year))
daylight_hours = daylight_hours(sha)
ird = inv_rel_dist_earth_sun(day_of_year)
et_rad = et_rad(deg2rad(latitude), sol_dec(day_of_year), sha, ird)
cs_rad = cs_rad(elevation, et_rad)

if sol_rad<1:
    sol_rad = sol_rad_from_sun_hours( daylight_hours, 0.8 * daylight_hours, et_rad )

net_in_sol_rad = net_in_sol_rad(sol_rad=sol_rad, albedo=0.23)     # (1 - albedo) * sol_rad

avp = avp_from_tdew(tdew)
#print("avp=",avp)

net_out_lw_rad = net_out_lw_rad(
        celsius2kelvin(temp_c_min),
        celsius2kelvin(temp_c_max),
        sol_rad,
        cs_rad,
        avp,
    )
net_rad = net_rad(net_in_sol_rad, net_out_lw_rad)
#logger.error("netrad=%f",net_rad)

svp=svp_from_t(temp_c)   # Saturation vapour pressure [kPa]. Can be estimated using ``svp_from_t()’‘.
delta_svp=delta_svp(temp_c)   # Slope of saturation vapour pressure curve [kPa degC-1]. Can be estimated using delta_svp().

psy=psy_const(atmos_pres / 10 )

#shf – Soil heat flux (G) [MJ m-2 day-1] (default is 0.0, which is reasonable for a daily or 10-day time steps). 

t=celsius2kelvin(temp_c)
#logger.error("args: %f %f %f %f %f %f %f",net_rad,t,ws,svp,avp,delta_svp,psy)
et = round( fao56_penman_monteith(net_rad, t, ws, svp, avp, delta_svp, psy, shf=0.0) , 2)
if et<0 or et>10: et=0

hass.states.set(
                "sensor.evapotranspiration", 
                et, 
                {"unit_of_measurement":"l/m²","friendly_name":"evapotranspiration","source":"custom python script evapotranspiration.py","icon":"mdi:weather-rainy"}
                )
#logger.error("et=%f",et)
