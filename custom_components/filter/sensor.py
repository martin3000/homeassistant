...
FILTER_NAME_TIME_MM = "time_moving_max"     # hüllkurve für sonneneinstrahlung
FILTER_NAME_TIME_MS = "time_moving_sum"     # kumuliert die erschütterungen der markise
FILTERS = Registry()
...
FILTER_TIME_MM_SCHEMA = FILTER_SCHEMA.extend(
    {
        vol.Required(CONF_FILTER_NAME): FILTER_NAME_TIME_MM,
        vol.Required(CONF_FILTER_WINDOW_SIZE): vol.All(
            cv.time_period, cv.positive_timedelta
        ),
    }
...
FILTER_TIME_MS_SCHEMA = FILTER_SCHEMA.extend(
    {
        vol.Required(CONF_FILTER_NAME): FILTER_NAME_TIME_MS,
        vol.Required(CONF_FILTER_WINDOW_SIZE): vol.All(
            cv.time_period, cv.positive_timedelta
        ),
    }
        ...
        vol.Required(CONF_FILTERS): vol.All(
            ...
            [
                vol.Any(
                    ...
                    FILTER_TIME_MM_SCHEMA,
                    FILTER_TIME_MS_SCHEMA,
...
# jms: time moving max filter
@FILTERS.register(FILTER_NAME_TIME_MM)
class TimeMMFilter(Filter, SensorEntity):
    """ Moving Max (MM) Filter.

    The window_size is determined by time.
    """

    def __init__(
        self, window_size, precision, entity
    ):  # pylint: disable=redefined-builtin
        """Initialize Filter.

        :param type: type of algorithm used to connect discrete values
        """
        super().__init__(FILTER_NAME_TIME_MM, window_size, precision, entity)
        self._time_window = window_size
        self.last_leak = None
        self.queue = deque()
    ...
    def _filter_state(self, new_state):
        """Implement the Moving Max filter."""
        ...
        prev_state = self.last_leak or self.queue[0]
        for state in self.queue:
            maximum = max(maximum, prev_state.state)
            prev_state = state

        new_state.state = maximum

        return new_state

# jms: time moving sum filter
@FILTERS.register(FILTER_NAME_TIME_MS)
class TimeMSFilter(Filter, SensorEntity):
    """ Moving Sum (MS) Filter.
    ...
    def __init__(
        ...
        :param type: type of algorithm used to connect discrete values
        """
        super().__init__(FILTER_NAME_TIME_MS, window_size, precision, entity)
        self._time_window = window_size
        self.last_leak = None
        self.queue = deque()
    ...
    def _filter_state(self, new_state):
        """Implement the Moving Sum filter."""
        ...
        prev_state = self.last_leak or self.queue[0]
        for state in self.queue:
            xsum = xsum + prev_state.state
            prev_state = state

        new_state.state = xsum
